#!/usr/bin/env bash
# PreToolUse guardrail (Edit|Write|MultiEdit|NotebookEdit): opt-in edit boundary.
# While a state file names a boundary directory, edits outside it are blocked.
#
...
#   arm:    CFG="${CLAUDE_CONFIG_DIR:-$HOME/.claude}"
...
#
# Honest limits: stops the file tools only, NOT Bash
...
STATE="${EDIT_BOUNDARY_FILE:-${CLAUDE_CONFIG_DIR:-$HOME/.claude}/hooks/state/edit-boundary}"

# Not armed: allow (the common case; keep it fast).
[ -f "$STATE" ] || exit 0
...
case "/$FILE/" in
  */../*)
    ...
    echo "BLOCKED: $FILE has an unresolved '..' component and cannot be verified against the edit boundary ($BOUNDARY). Create the parent directory or pass a fully resolved path." >&2
    exit 2 ;;
esac
...
echo "BLOCKED: $FILE is outside the armed edit boundary ($BOUNDARY). If this edit is intentional, disarm first: command rm \"${STATE}\"" >&2
exit 2

...
**Rule:** The final rewrite contains no em dashes (U+2014) or en dashes (U+2013). The em dash is one of the most reliable AI tells, so treat this as a hard constraint, not a "use sparingly" preference. Replace each one, in rough order of preference: a period (start a new sentence), a comma (a tight aside), a colon (introducing an explanation), parentheses (a true aside), or restructure the sentence. Also catch spaced em dashes (a space, U+2014, then a space) and double hyphens (` -- `) used the same way.

**Note on this file:** the harness forbids the literal U+2014 byte in any repo file, so the examples below write the em dash as `[U+2014]`. When you scan a draft, match the real character, not the marker.

**Before:**
> The term is primarily promoted by Dutch institutions[U+2014]not by the people themselves. You don't say "Netherlands, Europe" as an address[U+2014]yet this mislabeling continues[U+2014]even in official documents.

**After:**
> The term is primarily promoted by Dutch institutions, not by the people themselves. You don't say "Netherlands, Europe" as an address, yet this mislabeling continues in official documents.

**Before:**
> The new policy [U+2014] announced without warning [U+2014] affects thousands of workers. The changes -- long overdue according to critics -- will take effect immediately.

**After:**
> The new policy, announced without warning, affects thousands of workers. The changes, long overdue according to critics, will take effect immediately.

Before returning the final rewrite, scan it for U+2014 (the em dash) and U+2013 (the en dash). Any hit means the draft isn't done.
...
### 34. Kicker Density Across the Piece (local addition, not upstream)

**Problem:** Every closing line can individually pass the rules above and the piece still reads as machine-written, because EVERY paragraph lands on a polished, quotable closer. LLM paragraphs are setup-payoff units of near-identical weight: a few setup sentences, then a kicker (an aphorism, a moral, a contrast snap, a mic-drop fragment). A human writer lands one line like that per piece and does not notice the other paragraphs ended flat. This is a whole-piece check, not a sentence check: after auditing individual patterns, list the final sentence of each paragraph and count how many are engineered to be quotable. More than one or two in a short piece, or several in a row, is machine rhythm even when no single line matches any pattern in this guide.

**Fix:** Keep the single best kicker. Demote the rest: end those paragraphs on a plain fact, a number, a pointer, or simply stop after the last substantive sentence. Flat endings are the human default.

**Before (paragraph endings collected from one piece):**
> ...because a backup you never test is not a backup at all.
> ...it adds a few seconds per save, not a cost you notice.
> ...because a tool should be honest about what it can't do.
> ...open source, built to be extended.

**After:**
> ...a backup you never test will fail exactly when you need it. (the one kicker kept)
> ...it adds a few seconds per save. The benchmarks are in the docs. (flat: fact plus pointer)
> ...open source, extend it. (plain instruction)
...
- **One strong closing line.** A single well-earned kicker per piece is craft, not a tell. The tell is density: nearly every paragraph landing one (see §34).
...
2. Write a **draft rewrite**. Check that it reads naturally aloud, varies sentence length, prefers specific details and simple constructions (is/are/has), and keeps the appropriate register. Then list the final sentence of every paragraph and run the kicker-density check (§34): at most one engineered closer per piece.
